...
def parse_args():
    ...
    parser.add_argument('--raf_path', type=str, default='/mnt/home/lair/zhh/RAF-DB/', help='Raf-DB dataset path.')
...
class Res18Feature(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.drop_rate > 0:
            x =  nn.Dropout(self.drop_rate)(feature)
        else:
            x = feature
            x = x.view(x.size(0), -1)
        ...
        return attention_weights, out, feature
...
def run_training():
    ...
    for i in range(1, args.epochs + 1):
        ...
        iter_cnt = 0
        res18.eval()
        for batch_i, (imgs, targets, indexes, path) in enumerate(train_loader):
            batch_sz = imgs.size(0) 
            iter_cnt += 1
            tops = int(batch_sz* beta)
            optimizer.zero_grad()
            imgs = imgs.cuda()
            attention_weights, outputs, features = res18(imgs)
            
            global labels_list
            global feature_list
            labels_list = np.append(labels_list, targets.cpu().numpy())
            if np.shape(feature_list)[0] == 0:
                feature_list = features.cpu().detach().numpy()
            else:
                feature_list = np.vstack((feature_list, features.cpu().detach().numpy()))
            # Rank Regularization
        ...
        with torch.no_grad():
            ...
            if acc > 0.87 :
                ...
                print('Model saved.')
    #file_handle=open('label_tsne.txt',mode='w')
    #content = str(index_list)
    #file_handle.write(content)
    #file_handle.close()
    #file_handle=open('feature_for_tsne.txt',mode='w')
    #content2 = str(feature_list)
    #file_handle.write(content2)
    #file_handle.close()
    feature_list = feature_list[:,:,0,0]
    print(np.shape(labels_list), np.shape(feature_list))
    #print(labels_list)
#    print(feature_list)
...
titles = ['Surprise', 'Fear', 'Disgust', 'Happiness', 'Sadness', 'Anger', 'Neutral']
n_samples, n_features = feature_list.shape
'''t-SNE'''
tsne = manifold.TSNE(n_components=2, init='pca', random_state=501)
X_tsne = tsne.fit_transform(feature_list)
 
print("Org data dimension is {}.      Embedded data dimension is {}".format(feature_list.shape[-1], X_tsne.shape[-1]))
...
for i in range(X_norm.shape[0]):
    plt.text(X_norm[i, 0], X_norm[i, 1], str(titles[labels_list[i].astype(int)]), color=plt.cm.Set1(labels_list[i].astype(int)), 
             fontdict={'weight': 'bold', 'size': 9})
plt.xticks([])
plt.yticks([])
plt.show()
plt.savefig("./feature_clu.jpg")