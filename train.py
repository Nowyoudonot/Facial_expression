...
class RafDataSet(data.Dataset):
    def __init__(self, raf_path, phase, transform = None, basic_aug = False):
        ...
        NAME_COLUMN = 0
        LABEL_COLUMN = 1
        df = pd.read_csv(os.path.join(self.raf_path, 'EmoLabel/list_patition_label_six.txt'), sep=' ', header=None)
        if phase == 'train':
        ...
        for f in file_names:
            f = f.split(".")[0]
            f = f +"_aligned.jpg"
            path = os.path.join(self.raf_path, 'Image/sixteen', f)
...
#                train_loader.dataset.label[label_idx.cpu().numpy()] = relabels.cpu().numpy() # relabel samples in train_loader
        ...
        with torch.no_grad():
            ...
            for batch_i, (imgs, targets, _) in enumerate(val_loader):
                ...
                iter_cnt+=1
                _, predicts = torch.max(outputs, 1)
                #加一个batch起始编号的参数，将bad case的下表存在list中
